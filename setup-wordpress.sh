...
#       [+] Creating 2/0
#        ✔ Container db-dev  Running                                  0.0s
#        ✔ Container web-dev   Running                                  0.0s
#
set -o errexit
set -o errtrace
set -o nounset
...
check_requirements() {
    ...
    for _container in web-dev db-dev
    do
...
composer_install() {
    ...
    docker compose run --rm web-dev composer install --ansi 2>&1 \
        | sed \
...
environment_info() {
    ...
    print_key_val 'MariaDB version' \
        "$(echo; docker compose exec db-dev mariadb --version)"
    echo
...
check_user_permissions() {
    if ! docker compose exec -T --user "$WP_USER" \
        web-dev test -w "$WEB_WP_DIR"; then \
