...
delete_wordpress_uploads() {
    ...
    then
        ...
        # (xargs is used to trim whitespace)
        _count=$(docker compose exec web-dev \
...
import_uploads() {
    ...
    then
        ...
        docker compose cp "${CACHE_UPLOADS_DIR}" \
            "web-dev:${DOCKER_WP_UPLOADS_DIR}.temp" 2>/dev/null
        echo 'Set ownership of temp uploads dir to www-data:wwww-data'
        docker compose exec --user root web-dev chown -R www-data:www-data \
            "${DOCKER_WP_UPLOADS_TEMP_DIR}"
        echo 'Replace uploads dir with temp uploads dir'
        docker compose exec web-dev mv "${DOCKER_WP_UPLOADS_DIR}" \
            "${DOCKER_WP_UPLOADS_DIR}.old"
        docker compose exec web-dev mv "${DOCKER_WP_UPLOADS_DIR}.temp" \
            "${DOCKER_WP_UPLOADS_DIR}"
        docker compose exec web-dev rm -rf "${DOCKER_WP_UPLOADS_DIR}.old"
    elif [[ "${SCRIPT_ENV}" == 'server' ]]
    then
        print_var SERVER_WP_UPLOADS_DIR
        sudo_auth
...
script_setup() {
    ...
    case $(uname) in
        Darwin)
            ...
            for _service in web-dev db-dev
            do
            ...
            print_key_val 'WordPress PHP version' \
                "$(docker compose exec web-dev php --version \
                    | awk '/^PHP/ {print $2}')"
            ...
            then
                ...
                _err="${_err} ansible-dev repository (this check requires"
                _err="${_err} the current directory to me named"
                _err="${_err} 'ansible-dev')"
                error_exit "${_err}"
            fi
